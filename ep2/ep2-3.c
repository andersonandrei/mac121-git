...
int podeMover(int **tab, int p, int q, int mov) {
	if (tab[p][q] == -1) {
		...
		else if (mov == 2 && p < 1 && tab[p+1][q] == 1 && tab[p+2][q] == 1) return 1;
		else if (mov == 3 && q < 1 && tab[p][q+1] == 1 && tab[p][q+2] == 1) return 1;
		else return 0;
	}
	else {
		return 0;
...
int main() {
	...
	tab = criaMatriz(3);
	tabInversa = criaMatriz(3);
	
	inverteMatriz(tab,tabInversa,3);
	...
	iguais = comparaMatriz(tab,tabInversa,3);
